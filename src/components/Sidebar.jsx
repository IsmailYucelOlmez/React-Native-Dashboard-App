  ...
  return (
    ...
    {!settings ? (
      <View className='flex flex-col items-center gap-4 mt-5 z-30'>
        ...
        <Pressable onPress={()=>navigateScreen("KanbanScreen")} className={`flex flex-row py-1 w-3/4  rounded-xl items-center justify-center ${route.name === "kanban" ? `bg-[${themeColor}] ` : ''}`} ><MaterialIcons name="view-kanban" size={20} color={`${route.name=="kanban" ? 'white':'black'}`}/><Text  className={`ml-3 ${route.name==="kanban" ? 'text-white text-lg font-semibold':'text-base'} `}>Kanban</Text></Pressable>
